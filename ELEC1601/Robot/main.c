...
int main(int argc, char *argv[]) {
    ...
    int front_left_sensor=0;
    int left_short_sensor=0;
    int right_short_sensor= 0;
    int right_sensor=0;
    int left_sensor=0;
    ...
    while(!done){
        ...
        if (robot.auto_mode == 1){
            robotAutoMotorMove(&robot, front_right_sensor,front_left_sensor, left_short_sensor, right_short_sensor, right_sensor, left_sensor);
            robotMotorMove(&robot);
            // SDL_Surface * image = SDL_LoadBMP("star-wars-backgrounds-16.bmp");
        ...
        //Otherwise compute sensor information
            ...
            //RIGHT SENSOR START HERE

            left_short_sensor = checkRobotSensorLeftDiagonalAllWalls(&robot, head);
            right_short_sensor = checkRobotSensorRightDiagonalAllWalls(&robot, head);

            left_sensor = checkRobotSensorLeft(&robot, head);
            right_sensor = checkRobotSensorRight(&robot, head);
            // printf("SL:  %d,SR: %d,  FR:  %d,  FL:  %d, L %d, R %d\n", left_short_sensor, right_short_sensor, front_right_sensor,front_left_sensor, left_sensor, right_sensor);
            //END HERE
        }
        ...
        if (robot.auto_mode == 1){
            ...
            SDL_Surface * Speed1 = SDL_LoadBMP("speed1.bmp");
            SDL_Surface * Speed2 = SDL_LoadBMP("speed2.bmp");
            SDL_Surface * Speed3 = SDL_LoadBMP("speed3.bmp");
            SDL_Surface * Speed4 = SDL_LoadBMP("speed4.bmp");

            robotUpdate(renderer, &robot, Speed1, Speed2, Speed3, Speed4, front_left_sensor, front_right_sensor, left_short_sensor,right_short_sensor,left_sensor, right_sensor);
        ...
        /////////game page
                ...
                SDL_Surface * image = SDL_LoadBMP("start.bmp");
                SDL_Texture * texture = SDL_CreateTextureFromSurface(renderer, image);
                SDL_RenderCopy(renderer, texture, NULL, NULL);
        //////////welcome page
        }
